...
def create_json_file(text_list, output_file):
    ...
    for i, text in enumerate(text_list):
        json_entry = {
            "id3": f"hatespeech_{i}",  # 새로운 텍스트마다 고유한 id를 생성합니다. 여기서는 "hatespeech_숫자" 형태로 지정합니다.
...
output_file_path = "step3_hatespeech.json"  # 생성할 json 파일의 경로와 이름을 지정합니다.
